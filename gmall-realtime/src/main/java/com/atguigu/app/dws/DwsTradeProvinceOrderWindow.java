...
//        System.setProperty("HADOOP_USER_NAME", "atguigu");
        //TODO 2.读取dwd层order_detail数据，并去重过滤
        ...
        //TODO 4提取时间戳生成waterMark
        SingleOutputStreamOperator<TradeProvinceOrderWindow> tradeProvinceWithDS = tradeProvinceDS.assignTimestampsAndWatermarks(WatermarkStrategy.<TradeProvinceOrderWindow>forBoundedOutOfOrderness(Duration.ofSeconds(2)).
                withTimestampAssigner(new SerializableTimestampAssigner<TradeProvinceOrderWindow>() {
                    @Override
                    public long extractTimestamp(TradeProvinceOrderWindow element, long l) {
                        return element.getTs();
                    }
                }));

        //TODO 5.分组开窗聚合
        KeyedStream<TradeProvinceOrderWindow, String> keyedStream = tradeProvinceWithDS.keyBy(TradeProvinceOrderWindow::getProvinceId);
        ...
        SingleOutputStreamOperator<TradeProvinceOrderWindow> tradeProvinceOrderWindowProvinceNameDS = AsyncDataStream.unorderedWait(
                reduceDS,
        ...
        tradeProvinceOrderWindowProvinceNameDS.print(">>>>>>>>>>>>");
        tradeProvinceOrderWindowProvinceNameDS.addSink(MyClickHouseUtil.getClickHouseSink("insert into dws_trade_province_order_window values(?,?,?,?,?,?,?)"));
