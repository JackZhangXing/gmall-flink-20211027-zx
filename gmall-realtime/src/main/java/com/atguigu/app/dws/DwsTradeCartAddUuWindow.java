 ...
 * 启动zk.kf,mxw
 * 开启clickHouse.
 * 开启DwdTradeCartAdd程序，造业务数据。 (java -jar gmall2020-mock-db-2021-11-14.jar)
 */
...
//        System.setProperty("HADOOP_USER_NAME", "atguigu");
        ...
        String page_topic = "dwd_trade_cart_add";
        String groupId = "dws_trade_cart_add_uu_window_211027";
        DataStreamSource<String> cartAddStringDS = env.addSource(MyKafkaUtil.getKafkaConsumer(page_topic, groupId));
        //TODO 3.将每行数据转换为Json格式
        SingleOutputStreamOperator<JSONObject> jsonObjDS = cartAddStringDS.map(JSON::parseObject);


        //TODO 4.提取事件戳生成WaterMark
        ...
        SingleOutputStreamOperator<JSONObject> jsonObjWithWmDS = jsonObjDS.assignTimestampsAndWatermarks(WatermarkStrategy.<JSONObject>forBoundedOutOfOrderness(Duration.ofSeconds(2))
                .withTimestampAssigner(new SerializableTimestampAssigner<JSONObject>() {
                    @Override
                    public long extractTimestamp(JSONObject element, long recordTimpStamp) {
        ...
        SingleOutputStreamOperator<CartAddUuBean> cartAddDS = keyedStream.flatMap(new RichFlatMapFunction<JSONObject, CartAddUuBean>() {
            ...
            public void flatMap(JSONObject value, Collector<CartAddUuBean> out) throws Exception {
                ...
                //如果状态数据为null或者当前日期不是同一天,则保留数据，更新状态
        ...
        SingleOutputStreamOperator<CartAddUuBean> reduceDS = windowedStream.reduce(new ReduceFunction<CartAddUuBean>() {
            @Override
